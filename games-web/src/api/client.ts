...
// Agrupamento favoritável do domínio Games: a LOJA (Steam, GOG, Humble...).
// O núcleo expõe o mesmo endpoint genérico /api/favorite-groups.
export const favoriteStores = {
  getAll: () => request<string[]>('/api/favorite-groups'),

  add: (store: string) =>
    request(`/api/favorite-groups/${encodeURIComponent(store)}`, { method: 'POST' }),

  remove: (store: string) =>
    request(`/api/favorite-groups/${encodeURIComponent(store)}`, { method: 'DELETE' }),
}
